...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        // Design §4: flat "paper" cards — 1px #E7E1D3 border + whisper shadow, radius-md (14px).
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-md border border-border bg-card py-(--card-spacing) text-sm text-card-foreground shadow-[0_2px_8px_rgba(43,40,35,0.06)] [--card-spacing:--spacing(4)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-md *:[img:last-child]:rounded-b-md",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        // Design §2.1 H3: 18px SemiBold Jakarta Sans.
        "text-[18px] leading-[1.3] font-semibold group-data-[size=sm]/card:text-base",
        className
      )}
      {...props}
    />
