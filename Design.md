...
|1.3|**Phase 2 (Layanan Surat):** menambahkan semantik status surat (§1.3) yang telah dipetakan ke token `--status-*`; retensi PDF 7 hari (deviasi dari Master Doc §4, dicatat di `02_PHASE_2_LAYANAN_SURAT.md` §5.3).|
...
|1.5|**Iterasi UI Phase 2 (as-implemented):**<br/>- Consent gate terpusat menjadi satu `ConsentGate` card (ikon + teks ringkas) yang tampil inline di `/layanan-surat` & `/profil` — bukan dialog di halaman login.<br/>- Tabel antrian admin: pagination client-side (20/halaman), tombol nav icon chevron, banner retensi 7 hari selalu tampil, baris expired → tombol PDF disabled + catatan kecil di bawah tabel.<br/>- PDF surat: stempel/cap diperbesar dan digeser kiri sehingga bagian kanan stempel beririsan dengan bagian kiri tanda tangan (mengikuti konvensi surat desa); nama/NIP tetap pada posisi semula.|
